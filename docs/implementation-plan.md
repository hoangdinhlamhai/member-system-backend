...
> **Ngày tạo:** 2026-03-18
> **Cập nhật:** 2026-03-24
> **Trạng thái:** Active — Sprint 5 done, Sprint 1 partial
> **Tiến độ tổng:** ██████████░░░░░ ~65% MVP
...
| 18 | **Bill min 100k cho referral** | `src/webhook/`, `src/transactions/` |
| 19 | **Referral QR Code (deep link)** | ZMA `referral-code-card.tsx`, `auth-guard.tsx`, `login.tsx` |
| 20 | **Dashboard doanh số API fix** | `src/members/members.service.ts` (auto_approved + finalAmount) |
| 21 | **Voucher QR overlay** | ZMA `my-vouchers-tab.tsx` (qrcode.react) |
| 22 | **Sileo toast notifications** | ZMA `layout.tsx`, `login.tsx` |
| 23 | **UI safe area fix (header/footer)** | ZMA `variables.scss`, `app.scss`, all pages |
...
| ✅ Done  | Tier config align PRD             | S      | ~~1~~ Done |
| ✅ Done  | Zalo OAuth 2.0 (trusted frontend) | L      | ~~4~~ Done |
| ✅ Done  | Bill min 100k cho referral        | S      | ~~1~~ Done |
| ✅ Done  | Rewards/Vouchers API              | L      | ~~2~~ Done |
| ✅ Done  | Web Admin Dashboard (Next.js)     | XL     | ~~5~~ Done |
...
> **Mục tiêu:** Align business rules với PRD
> **Effort:** ~1 ngày
> **Trạng thái:** ✅ Done

### Task 1.1 — Update Tier Seed Data ✅ DONE
...
### Task 1.2 — Business Rule: Bill tối thiểu 100k cho referral ✅ DONE

**Files:**

- `src/webhook/webhook.service.ts` → `processReferralEngine()` + `activateReferralIfPending()`
- `src/transactions/transactions.service.ts` → `approveBill()` referral section

**Logic đã implement:**

- Bill < 100k → Skip referral hoàn toàn (giữ pending, KHÔNG activate, KHÔNG tính hoa hồng)
- Bill >= 100k → Activate referral (pending → active) + tính 5% hoa hồng
- Doanh số vẫn cộng bình thường cho mọi bill bất kể giá trị

---

### Sprint 1 — Checklist

- [x] 1.1 Update tier seed data ✅
- [x] 1.2 Bill min 100k cho referral ✅
- [x] Test webhook flow ✅
- [x] Test approve flow ✅
...
> **Effort:** ~2 ngày
> **Dependency:** Sprint 1 xong
> **Trạng thái:** 🟡 Core done — Staff quét voucher còn thiếu
...
- [x] 2.1 Tạo `RewardsModule` (module, controller, service) ✅
- [x] 2.2 `GET /rewards/catalog` + `GET /rewards/my-vouchers` ✅
- [x] 2.3 `POST /rewards/redeem` (atomic, voucher code gen) ✅
- [ ] 2.4 `POST /staff/scan-voucher`
- [x] 2.5 Seed data (discount tiers + sample rewards) ✅
- [x] 2.6 Frontend: replace mock với API thật ✅
...
> **Trạng thái:** ✅ Done — All core pages
