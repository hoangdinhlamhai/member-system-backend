...
[
  {
    ...
    "voucherCode": "ZDC-D-1234",
    "qrData": "ZDC:VOUCHER:ZDC-D-1234",
    "status": "active",
...
Webhook hứng sự kiện từ Zalo OA. **Không cần auth.**

- Trả 200 OK ngay lập tức (fire-and-forget)
- Xử lý async: keyword match → query DB → reply via OA CS API
- **KHÔNG dùng DTO validation** (`@Body() body: any`) — webhook phải luôn 200
...
```json
{
  ...
  "sender": { "id": "OA_SCOPED_ID" },
  "user_id_by_app": "APP_SCOPED_ID (= zaloId in DB)",
  "message": { "text": "doanh so" }
}
```

> ⚠️ `sender.id` (OA-scoped) dùng để reply. `user_id_by_app` (App-scoped) dùng để query member.
...
| Method | Path                           | Mô tả                                       |
| ------ | ------------------------------ | ------------------------------------------- |
| POST   | `/admin/zalo-oa/init-token`    | Khởi tạo bằng refresh_token → gọi Zalo API  |
| POST   | `/admin/zalo-oa/save-token`    | Lưu trực tiếp cặp token từ Console          |
| GET    | `/admin/zalo-oa/token-status`  | Xem trạng thái (hasToken, expiresInMinutes) |
| POST   | `/admin/zalo-oa/refresh-token` | Force refresh ngay lập tức                  |

**POST /admin/zalo-oa/save-token Body:**
...
**GET /admin/zalo-oa/token-status Response:**
